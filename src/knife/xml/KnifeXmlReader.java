        ...
        while (reader.hasNext()) {
            ...
            case XMLStreamConstants.START_ELEMENT:
                ...
                if ("knife".equals(tag)) {
                    ...
                    knife.getVisual().getBlade().setLength(Integer.parseInt(reader.getElementText()));
                } else if("width".equals(tag)) {
                    knife.getVisual().getBlade().setWidht(Integer.parseInt(reader.getElementText()));
                } else if("blade_material".equals(tag)) {
                    knife.getVisual().getBlade().setMaterial(BladeMaterial.valueOf(reader.getElementText()));
                } else if("fuller".equals(tag)) {
                    knife.getVisual().getBlade().setFuller(Boolean.parseBoolean(reader.getElementText()));
                } else if("handle_material".equals(tag)) {
                    knife.getVisual().getHandle().setMaterial(HandleMaterial.valueOf(reader.getElementText()));
                } else if("material_type".equals(tag)) {
                    knife.getVisual().getHandle().setType(reader.getElementText());
